...
class AdminDAO{
	...
	public function inserir($admin){
		include("conexao.php");
		$sql = 'INSERT INTO admin (id, img, login, senha) VALUES (:id, :img, :login, :senha)';
		$consulta = $conexao->prepare($sql);
		$consulta->bindValue(':id',$admin->getId()); 
		$consulta->bindValue(':img',$admin->getImg()); 
	...
	public function atualizar($admin){
		include("conexao.php");
		$sql = 'UPDATE admin SET id = :id, img = :img, login = :login, senha = :senha WHERE id = :id';
		$consulta = $conexao->prepare($sql);
		$consulta->bindValue(':id',$admin->getId()); 
		$consulta->bindValue(':img',$admin->getImg()); 
		$consulta->bindValue(':login',$admin->getLogin()); 
